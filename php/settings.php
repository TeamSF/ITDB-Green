...
if (isset($_POST['dateformat']) ) { //if we came from a post (save), update the rack 
  ...
  "', ldap_bindpass='".base64_encode($_POST['ldap_bindpass']).
...
-->
    ...
    <tr><td class="tdt"><?php te("LDAP Bind Pass");?>:</td>
        <td><input  class='input2 ' size=20 type=password name='ldap_bindpass' value="<?php echo base64_decode($settings['ldap_bindpass'])?>"> Password for the Bind User</td></tr>
...
<?php if ($settings['useldap']==1) { ?>
    ...
    <tr><td class="tdt"><?php te("Use LDAP User & Item Sync");?>:</td>
        ...
        <?php
        if ($settings['useldapsync']==1) $s1='SELECTED';
        else $s1='';
        ?>
        <option value=0><?php echo t('No')?></option>
        <option <?php echo $s1?> value=1><?php echo t('Yes')?></option>
