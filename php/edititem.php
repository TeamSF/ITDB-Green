...
//ldap form variables
$ldap_formvars=array("dn","ou_full","ou_name","os","os_servicepack","os_version");
...
//get form post variables and create the sql query
  ...
  //Get old dnsname and compare with current dnsname
  $sql="SELECT dnsname from items where id=$id";
  $sth=db_execute($dbh,$sql);
  $olddnsname=$sth->fetchAll(PDO::FETCH_ASSOC);
  $olddnsname=$olddnsname[0];

  //Reset sync time when hostname was changed
  //Helps to get new ldap info by skipping sync delay
  if($dnsname != $olddnsname['dnsname']) $reset_lastsync=TRUE;
  else $reset_lastsync=FALSE;

  $sql="UPDATE items set $set WHERE id=$id";
  db_exec($dbh,$sql); 

  //Save Active Directory and OS information, but only if LDAP check succeeded!
  //When itemid is found and matches id continue with update, otherwise insert
  if(isset($_POST['ldap_check']) && $_POST['ldap_check'] == 1 || $reset_lastsync) {
    $set_ldap="";
    foreach ($ldap_formvars as $ldap_formvar){
        if (isset($_POST[$ldap_formvar]))
          $$ldap_formvar=trim($_POST[$ldap_formvar]);
        else {$i++;continue;}
        ...
        if ($$ldap_formvar == "") {
          ...
          $set_ldap.="$ldap_formvar='".htmlspecialchars($$ldap_formvar,ENT_QUOTES,"UTF-8")."', ";
        }
        $i++;
    }

    //Update or insert LDAP information
    if(isset($_POST['found_id']) && $_POST['found_id'] == $id)
    {
        if($reset_lastsync) $set_ldap.="lastsync=0";
        else $set_ldap.="lastsync=".time();
        $sql="UPDATE item2ldap set $set_ldap WHERE itemid=$id";
        db_exec($dbh,$sql);
    }
    else {
        if($reset_lastsync) $lastsync=0;
        else $lastsync=time();
